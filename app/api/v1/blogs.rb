  ...
  class Blogs < Grape::API
    ...
    namespace 'blog/posts' do
      ...
      get do
        ...
        orderby = (params[:orderby].present? && params[:orderby] == 1) ? 'created_at desc' : 'comment_count desc, views desc, id desc'

        privacy_levels = []
        authenticate_user
        if authenticated?
          privacy_levels = [::BlogPost.privacies[:everyone], ::BlogPost.privacies[:members_only]]
        else
          privacy_levels = [::BlogPost.privacies[:everyone]]
        end

        privacy_levels = [::BlogPost.privacies[:everyone], ::BlogPost.privacies[:members_only]]

        unless params[:year].nil? || params[:month].nil?
          posts = ::BlogPost.where('extract(year  from created_at) = ? AND extract(month  from created_at) = ? ', params[:year], params[:month]).where(:status=>::BlogPost.statuses[:enable], :privacy=>privacy_levels).order(orderby, id: :desc).page(params[:page]).per(params[:limit])
        else
          if params[:search].present?

            posts = ::BlogPost.where(blog_id: blog.id).blog_search("%#{params[:search].downcase}%").where(:status=>::BlogPost.statuses[:enable], :privacy=>privacy_levels).order(orderby, id: :asc).page(params[:page]).per(params[:limit])
          else
            posts = ::BlogPost.where(blog_id: blog.id).where(:status=>::BlogPost.statuses[:enable], :privacy=>privacy_levels).order(orderby).page(params[:page]).per(params[:limit])
          end
        end
      ...
      params do
        ...
        optional :tags, type: JSON do
          requires :name, type: String
        end
        optional :categories, type: JSON do
        ...
        if authenticated?
          ...
          if !blog.nil?
            ...
            unless params[:tags].nil?
              tag_names = params[:tags].map { |tag| tag.name }
              tag_ids = Tag.multiple_save(tag_names)
            end

            _categories =[]
            unless params[:categories].nil?
              _categories = params[:categories].map { |cat| cat.id }
            end

            post = BlogPost.new(user_id: current_user.id, content: params[:content], status: post_status, privacy: privacy, blog_id: blog.id, title: params[:title], summary: params[:tagline], tag_ids: tag_ids, blog_category_ids: _categories)
            ...
            if post.save!
              ...
              if !params[:image].nil?
                ...
                if post.save!
                  if post_status == ::BlogPost.statuses[:pending]
                    response = { status: 202, message: "Blog post added to pending list", content: Entities::BlogPost.represent(post, current_user: current_user, commnet_limit: 5) }
                  else
                    response = { status: 201, message: 'Blog Post added', content: Entities::BlogPost.represent(post, current_user: current_user, commnet_limit: 5) }
                  end
                else
                  post.destroy
                  response = { status: 500, message: 'Failed to add the blog post banner', content: nil }
                end
              else
                if post_status == ::BlogPost.statuses[:pending]
                  response = { status: 202, message: "Blog post added to pending list", content: Entities::BlogPost.represent(post, current_user: current_user, commnet_limit: 5) }
              ...
              response = { status: 500, message: 'Failed to add blog post', content: nil }
            end
          else
            response = { status: 404, message: 'Blog not found', content: nil }
          end
        end
    ...
    namespace 'blog/archive' do
      get do
          ...
          resources = ::BlogPost.where(blog_id: blog.id, :privacy=>privacy_levels, status: ::BlogPost.statuses[:enable]).order('created_at desc')
      ...
      namespace 'blog/post/:post_id' do
      ...
      get do
        ...
        posts = ::BlogPost.where(id: params[:post_id], :privacy=>privacy_levels, status: ::BlogPost.statuses[:enable])
    ...
    namespace 'post/:slug' do
      ...
      get do
        ...
        # post = ::BlogPost.find_by(slug: params[:slug], :privacy=>privacy_levels, status: ::BlogPost.statuses[:enable])
        post = ::BlogPost.find_by(slug: params[:slug], status: ::BlogPost.statuses[:enable])
        
        if !post.nil?
          if privacy_levels.include? ::BlogPost.privacies[post.privacy]
            post.views = post.views.to_i + 1
            post.save!
            response = { status: 200, message: 'Blog posts found', content: Entities::BlogPost::Detail.represent(post, current_user: current_user, commnet_limit: commentLimit) }
          else
            response = { status: 401, message: 'Unautharized access', content: nil }
          end
        else
          response = { status: 404, message: 'Resource not found', content: nil }
        end
    ...
    namespace 'blog/post/:post_id/attachment/:id' do
      params do
        requires :post_id, type: Integer
    ...
    namespace 'blog/post/:post_id/comments' do
      params do
        requires :post_id, type: Integer
      ...
      get do
        ...
        authenticate_user
        if authenticated?
          post = ::BlogPost.find_by(id: params[:post_id])
        else
          post = ::BlogPost.find_by(id: params[:post_id], :privacy=>::BlogPost.privacies[:everyone])
        end

        if !post.nil?
          comments = ::BlogPostComment.where(blog_post_id: params[:post_id], parent_id: params[:parentId]).order(created_at: :desc, id: :desc).page(params[:page]).per(params[:limit])
          ...
          response = {status: 404, message: "Post not found", content:nil}
        end
    ...
    namespace 'blog/post/:post_id/comments/:id' do
    ...
    namespace 'blog/meta' do
      get do
        return {
          status: 200,
          message: 'blog meta data',
          content: {
    ...
    namespace 'blog/tag-search' do
      params do
        requires :name, type: String
      end
      get do
        tags = Tag.where("lower(name) LIKE ?", "%#{params[:name].downcase}%").select(:id , :name)
        if tags.blank?
          return {
            status: 404,
