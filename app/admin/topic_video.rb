...
ActiveAdmin.register LearngingLibrary, as: 'TopicResource' do
  belongs_to :sub_topic_category
  menu false
  navigation_menu do
    :default
  end
  ...
  form do |f|
    f.inputs 'Topic Resource Information' do
      f.input :title
    ...
    f.actions do
      if controller.action_name == 'new'
        f.action(:submit, label: 'Create Topic Resource')
      else
        f.action(:submit, label: 'Update Topic Resource')
      end
